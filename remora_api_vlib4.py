...
# data locations
can_pod5_path = '/home/marchandlab/xenomorph/xemora-beta/xemora-test/230625_PZ_libv2_analysis/modified/pod5/fast5.pod5'
can_bam_path = '/home/marchandlab/xenomorph/xemora-beta/xemora-test/230625_PZ_libv2_analysis/canonical/bam/bam.bam'
bed_path = '/home/marchandlab/xenomorph/xemora-beta/xemora-test/230625_PZ_libv2_analysis/references/P.bed'
fasta_path = '/home/marchandlab/xenomorph/xemora-beta/xemora-test/230625_PZ_libv2_analysis/references/xref_libv2_PZ_CxDx.fa'
output_folder= '/home/marchandlab/xenomorph/xemora-beta/xemora-test/230625_PZ_libv2_analysis/230628PZlib_levels.csv'
...
lev = pd.read_csv('/home/marchandlab/xenomorph/xemora-beta/xemora-test/230625_PZ_libv2_analysis/230628PZlib_levels_100k.csv')


total_ref = (len(list(set(lev['Reference_seq']))))
total_hep = (len(list(set(lev['read_xna_sequence']))))


print('# Reference sequences mapped: '+str(total_ref))
print('# Heptamer sequences assigned: '+str(total_hep))
...
if 1>0: 
    ...
    if gen_bai==True:
        ...
        os.system(cmd)
        #cmd = 'samtools index '+mod_bam_path
        #os.system(cmd)
    ...
    #output_summary_temp = pd.DataFrame(columns = column_names)


    #print(len(list(set(fasta_sequences))))



####Note - This section is poorly written and not scalable. Should pre-allocate memory. 
    j=0
    match_score = 0
            ...
            try:
               ...
               # if skip_bam_index == False: 
                ...
                if int(match_quality) >=match_score:
                    ...
                    #if len(output_summary_temp) % 1000 ==0: 
                    #    output_summary.loc[len(output_summary)] = output_summary_temp
                    #    output_summary_temp = pd.DataFrame(columns = column_names)
    ...
    '''
    df_p= output_summary[output_summary['strand'] =='+']
    df_m= output_summary[output_summary['strand'] == '-']

    df_plus = pd.DataFrame(df_p['Heptamer_level'].to_list(), columns=['1','2','3','4','5','6','7'])
    df_minus = pd.DataFrame(df_m['Heptamer_level'].to_list(), columns=['1','2','3','4','5','6','7'])
